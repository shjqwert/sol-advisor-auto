# Sol Advisor 3.0 架构设计

日期：2026-09-23。状态：`3.0.0` 源码静态验证完成（符号链接用例因宿主权限跳过）；后续经用户授权完成全局安装，但触发专项验收尚未通过。本文第 1–8 节说明当前设计，后半部分完整保留 2.0 历史设计与当时证据，不能用于证明 3.0 当前行为。

源码升级阶段是 source-only；其后用户单独授权安装和真实触发验收，并授权同步剩余升级内容到 Git、更新 Codex 缓存及发布 `3.0.0` 标签，执行记录见第 7 节。标签不代表原生行为专项验收通过。编译、完整构建、硬件和其他高成本操作仍受各自授权限制。文档说明设计和已有证据，不覆盖系统、应用、用户或项目权限。

## 1. 当前目标与边界

Sol Advisor 3.0 保留主会话协调、Scout 取证、Worker 实现、Reviewer 独立审查三种职责，把四个模型绑定配置收敛为三个模型无关的原生角色模板。模板只设置 OpenAI provider，不固定模型或 reasoning effort；主会话在每次 spawn 时显式选择二者。

插件不实现运行时调度器、消息总线、结果 sidecar、跨任务状态库或递归代理树。它也不要求数值评分、成功概率、派发报告、预检脚本或 Scout → Worker → Reviewer 固定流水线。主会话保留任务分解、范围和接口决定、最终整合及用户沟通责任。

完成条件是当前 Skill、三个角色模板、安装迁移、静态检查与文档互相一致。真实原生验证必须在受权安装后，由新任务重新加载自定义 v3 角色；旧角色、通用 GPT-6 代理或静态检查都不能替代该证据。

## 2. 当前委派判据

以下模式适合考虑有界委派：

- 隔离大量搜索结果、日志或中间阅读，避免挤占主上下文。
- 应用专门指令、工具或不同执行边界。
- 独立核对重要结果，或分别调查互相竞争的假设。
- 分配彼此可分离的模块、源码范围或调查维度。
- 将清晰、重复、适合较低成本模型的工作独立交付。

模式匹配且任务边界清楚后，仍须同时满足所需质量和可接受的总成本。总成本包括启动、上下文、推理、通信、等待、复核、整合和返工；不能在执行前宣称已证明质量或耗时收益。速度本身不是默认触发条件。频繁交互、关键共享上下文或昂贵交接通常更适合主会话。简单任务留在主会话，不为使用代理而创造工作或重复调查。

插件不限制代理数量；只为不同且有用的任务增加实例，并服从宿主并发、可用额度以及用户期限和预算。

## 3. 当前角色、模型与 effort

| 原生角色 | 职责 |
|---|---|
| `sol_advisor_scout` | 只读取证、定向研究与证据综合 |
| `sol_advisor_worker` | 自有范围实现、局部设计、调试与已授权检查 |
| `sol_advisor_reviewer` | 独立检查产物、主张和反例；不负责修复 |

三种角色均可使用 `gpt-6-luna`、`gpt-6-sol` 或 `gpt-6-astra`。三个 TOML 均只声明 `model_provider = "openai"`，不写死 `model` 或 `model_reasoning_effort`。派发时显式传入模型和 effort，并以 `<task>__<model_id>` 命名实例，例如 `trace_boot__gpt_6_luna`。

| 任务 | 起始选择 | 困难任务调整 |
|---|---|---|
| 明确查找、提取、证据整理 | Luna high | 困难追踪用 xhigh；复杂综合可改用 Sol |
| 常规实现、局部设计、调试 | Sol medium | 多步依赖和边界情况用 high |
| 独立审查 | Sol high | 约束冲突或困难反例用 xhigh |
| 特别困难的综合 | 比较高 effort Sol 与 Astra | Astra 从 low 起步，按实际难度改为 medium/high/xhigh |
| 异常困难的推理 | 所选模型支持的 max | 必须指出具体推理障碍，不能只以角色或重要性为由 |

Luna 在 v3 中只允许 high/xhigh/max；Sol 和 Astra 允许 low/medium/high/xhigh/max。none 和 ultra 不属于 v3 路由策略，实际选择还必须得到宿主支持。缺事实时先取证，不能用提高 effort 替代证据。无需强制从低 effort 逐级尝试，也不为普通派发运行 benchmark。

effort 的选择同时看推理链长度、交叉依赖、冲突证据和边界情况；若结论会影响多项后续工作，且错误难以及时暴露，应提高 effort。公共接口、基础状态语义等会扩散错误的决定，可以同时提高 effort 和独立验证强度。

## 4. 当前协作与生命周期

派发至少说明目标或交付物、自有范围、必要上下文、验收条件和协作者或限制。窄任务优先 fresh context；只有确需背景且宿主支持时才继承必要历史。子代理直接完成被分配工作，不创建或管理其他代理。

宿主暴露原生消息时，直接调用 `collaboration.send_message`；它不位于 `functions.exec` 或其 `ALL_TOOLS` 中。队友之间发送相关证据、问题和接口约定。消息不扩大授权、不转移 ownership，也不会唤醒已经结束的代理；后者由主会话安排 follow-up。若直接消息不可用，子代理通过普通 final 返回证据，由主会话保留来源后中转。不得用 app thread message 替代原生代理消息，也不等待不存在的通道。

同一子代理的 follow-up 保持原模型和 effort。v3 不支持在运行中的代理切换配置，也不承诺提示词缓存复用。若需要另一配置，先结束旧 ownership 再创建新任务，或由主会话确认旧执行停止、检查部分产物并接管。一个冲突文件、共享输出或独占设备同时只有一个负责人；ownership 是协作规则，不是安全沙箱。

结果使用普通 final，说明完成内容、关键证据和未完成项，不要求 STATUS/VERDICT 等固定字段。主会话检查实际改动和验收条件，只重验受变化影响的输入；没有执行的检查必须保持未验证状态。

## 5. 当前权限与降级

委派继承系统、应用、用户和项目限制，不能授权构建、硬件、安装、发布或其他副作用。角色提示中的只读和 ownership 是行为边界。Sol Advisor 不写用户或项目的 `AGENTS.md`，也不写 `.agent` 下的上下文、授权、计划或交接文件。

schema-v1 `.agent/authorizations.json` 中 `authorizations.solAdvisor.implicitDelegation = false` 关闭隐式委派；缺少文件、缺少键或值为 true 时允许考虑。无效或不可读配置降级为主会话执行。用户在当前任务中的明确委派可覆盖项目 opt-out，但不能覆盖更高优先级限制。缺少角色、模型、消息工具或额度时继续由主会话完成，不能静默替换配置或自行安装能力。
...
plugins/sol-advisor/
  ...
  scripts/
    ...
    validate-agent-route.sh
    inspect-agent-runtime.sh
    verify.sh
    fixtures/  历史受管模板输入
```

普通安装创建三个无绑定模板。受管升级精确识别已知 `2.0.1` 四配置：`sol_advisor_scout__gpt_5_6_luna`、`sol_advisor_worker__gpt_5_6_sol`、`sol_advisor_reviewer__gpt_5_6_sol`、`sol_advisor_reviewer__gpt_6_astra`，并在同一事务中迁移为三个 v3 角色。已知更早模板继续由历史 fixtures 支持。

迁移先核对所有目标和退役文件内容。任一受管目标被用户修改、内容未知或路径异常时，在写入前整体停止；不覆盖修改，也不留下部分升级。失败恢复保留原历史文件内容及 LF/CRLF 换行。无关自定义角色不受影响。该恢复只保证安装事务失败后的还原，不承诺任意版本降级。

## 7. 当前验证计划与已有证据

发布同步：在用户授权下补齐 Unicode JSONL 诊断修复后，Codex 注册及启用状态检查通过；53 个插件文件、3 个原生角色与源码逐文件一致。源码缓存更新不改变已运行会话的角色清单，原生行为验收仍需新任务加载。

2026-09-23 后续触发专项：经当前用户授权，迁移全局原生角色并同步 3.0.0 缓存；注册及启用状态、52 个插件文件、3 个角色文件的一致性检查通过。设计了 [21 个场景与测试问题](../tests/trigger-acceptance-2026-09-23.json)，执行记录见 [专项结果](../tests/trigger-acceptance-results-2026-09-23.json)。结论为 **尚未通过专项验收**：6 项有限 CLI 行为观察通过、3 项证据不足、12 项受运行环境或夹具可读性阻塞。

该次执行使用 CLI 0.156.0，请求主模型 Sol medium；CLI 默认 multi_agent_v2=false，因此默认协议结果不能替代 Desktop/V2 验收。仅对测试进程启用原生 V2 后，显式委派最小用例仍在模型行为前以 0xC0000409 退出，未证明新角色创建、实际模型/effort、点对点通讯或续派。接口清单夹具在宿主可读为文本，在测试任务中被读为 %TSD-Header-###% 内容，原因尚未确定。未为了使测试通过而修改编排规则，未关闭证书验证或更改全局功能开关。

显式 Skill 读取与五类不派发反例有事件证据；日志调查和简单反例核验未观察到隐式 Skill 调用。项目退出用例虽然未派发，但未观察到策略读取，不能证明退出机制起作用。T01 的独立复核表述缺少对应子代理身份事件，不作为派发成功证据。下一阶段应在重新加载的 Desktop 新任务中先确认三个 v3 角色与一个真实创建记录，再补正向触发及策略、通讯、续派变体；小型确定性夹具不宜强行要求派发。

静态验证应覆盖三个无绑定模板、角色/模型/effort 组合拒绝、受管迁移及失败恢复、历史 fixture、诊断字段和 Shell LF。它不调用模型或编译，也不能证明原生角色加载、消息、权限或行为。

升级阶段源码验证：`verify.sh` 完整通过；配置 4 项、合成运行诊断 6 项、安装 8 项、快照 2 项，共 20 项 unittest 中 19 项通过、1 项跳过。跳过项是目标及祖先目录符号链接测试，本机创建链接返回 WinError 1314；不得将该项视为已验证。入口额外的全新/重复安装、历史迁移、CRLF 转换、搜索预检、Shell 语法与 LF 检查通过；Skill 元数据和本地文档链接校验通过。

发布阶段补充回归：修复 JSONL 按 Unicode 行分隔字符错误拆分的问题，增加 3 项测试，覆盖 LF/CRLF、合法 Unicode 内容、非法物理换行、配置漂移及输出字段限制。发布候选共 23 项 unittest，22 项通过、1 项因上述链接权限限制跳过；没有进行编译、硬件或成本基准测试。

验证环境使用 Windows 本机 Python 3.13，并在 Git Shell 内将 `/usr/bin` 放到 PATH 前部；默认 MSYS Python 与 Windows PATH 混用曾调用 Windows `find.exe`，该环境错误在显式选择解释器和 POSIX 工具后消除。所有安装回归仅写入仓库内临时目标，未更新全局安装或插件缓存。

运行时诊断兼容旧版顶层 identity 字段和 v2 的 `source.subagent.thread_spawn` 嵌套 identity。`agent_role`、`model`、`effort`、`cwd` 必须存在、非空且在有关记录中一致；缺失、歧义或冲突时拒绝输出。`parent_thread_id`、`agent_path`、`model_provider` 为可选字段，缺失时输出 null；同一 parent/path 同时出现在顶层和嵌套结构时必须一致。诊断只输出 allowlist 字段，不读取它们来改变派发或 ownership。

经单独授权安装并新建任务后，v3 smoke test 应使用三个自定义角色，覆盖角色发现、显式模型/effort、直接证据传递、有界实现、叶子行为和主会话接管。报告实际宿主、模型、effort 与未覆盖项；旧 role name 或通用代理测试不能算新模板通过。

2026-09-23 的通用 `gpt-6-luna` 测试属于前置环境证据：两个 high 代理的直接 A731/B842 ping 与 acknowledgement 通过，第三个 xhigh 成功创建，日志确认三个实例的模型/effort。该测试没有加载 v3 自定义角色，也没有验证同一子代理切换模型或缓存行为。

当时 Codex Desktop 测试宿主为 `0.155.0-alpha.2.6`、协议 v2；standalone CLI `0.156.0` 已包含 appserver effort 接口，但仍有 v2 ownership 限制。v3 不集成 appserver，也不据此声明 Desktop 已支持所有配置。

## 8. 当前参考依据

- [OpenAI 子代理配置](https://learn.chatgpt.com/docs/agent-configuration/subagents)：给出 Luna high、Sol medium、Astra low 的基础起点，并说明自定义角色与原生子代理配置；本文仍以实际宿主支持和新会话测试为准。
- [Introducing GPT-6 Sol and Luna](https://openai.com/index/introducing-gpt-6-sol-and-luna/)：AutomationBench 中 Sol xhigh 为 33.2、Astra low 为 30.3；这是工作流 benchmark，不是 embedded 项目质量证明。DeepSWE 1.1 中 Sol max 为 68.8、Luna max 为 66.6，用作长时程编码参考，不自动改变路由默认。
- [Claude Code subagents](https://code.claude.com/docs/en/sub-agents) 与 [agent teams](https://code.claude.com/docs/en/agent-teams)：参考上下文隔离、专门化、独立任务与协作边界，不复制其宿主特有生命周期。
- [LangChain Deep Agents subagents](https://docs.langchain.com/oss/python/deepagents/subagents)：参考上下文隔离、专业能力、独立模块和成本适配等 task patterns。

公开价格与 benchmark 只帮助形成待验证选择，不保证订阅配额、实际总成本、缓存命中、任务速度或工程质量。

---

## 2.0 历史设计与历史证据（保留归档）

以下原第 1–14 节记录 2026-09-10 起的 2.0 设计、实施状态和宿主观察。它们仅解释 v3 的迁移来源与历史限制；其中角色名、模型、版本、当前状态和验证结论均不得作为 3.0 现状。原第 14 节的失败与通信证据按要求保留。
